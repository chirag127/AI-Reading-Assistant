...
function createToolbar() {
  ...
  document.getElementById('ai-summarize').addEventListener('click', summarizeArticle);
  document.getElementById('ai-keypoints').addEventListener('click', extractKeyPoints);
  document.getElementById('ai-explain').addEventListener('click', explainSelection);
  ...
  document.getElementById('ai-toggle').addEventListener('click', toggleToolbar);
...
function createPopupContainer() {
  ...
  if (!container) {
    ...
    document.getElementById('ai-popup-tts').addEventListener('click', () => {
      const content = document.querySelector('.popup-content').textContent;
...
function getPageContent() {
  ...
  const mainContent = document.querySelector('main, #content, .content, .article, .post');
...
function extractKeyPoints() {
  ...
  chrome.runtime.sendMessage({
    ...
    if (response && response.keyPoints && response.keyPoints.length > 0) {
...
function explainSelection() {
  ...
  chrome.runtime.sendMessage({
    ...
    const explanationContent = popup.querySelector('.explanation-content');
...
function init() {
  ...
  chrome.storage.sync.get({ toolbarCollapsed: false }, function(items) {
    if (items.toolbarCollapsed) {
      document.querySelector('.ai-reading-assistant-toolbar').classList.add('collapsed');
    }
  });
  ...
  chrome.storage.sync.get({ autoPopup: false }, function(items) {
    if (items.autoPopup) {
      ...
      const isNewsOrBlog = /news|blog|article/i.test(window.location.href) ||
